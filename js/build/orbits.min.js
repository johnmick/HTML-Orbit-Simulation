...
f,j=false;m=function(b){a=k.canvas.getContext("2d");f=new Image;f.src=b.earthimg;f.onload=function(){j=true};return m};m.redraw=function(){a.clearRect(0,0,k.canvas.width,k.canvas.height);j===false?c({x:e.x,y:e.y,radius:e.r,strokeStyle:"#00FF00",fillStyle:"#00FF00"}):a.drawImage(f,e.x-f.width/2,e.y-f.height/2);for(var b in n)n[b].draw()};m.drawSatellite=function(b){c(b);var g;for(g=b.xpoints.length-1;g>-1;g--)c({x:b.xpoints[g],y:b.ypoints[g],radius:1,fillStyle:b.colors[g]})}})();(function(){p=function(){return p};
p.reCalc=function(){for(var c in n){var a=n[c],f=Math.atan2(a.y-e.y,a.x-e.x),j=-6.673E-11*e.m*a.m/((a.x+e.x)*(a.x+e.x)+(a.y+e.y)*(a.y+e.y));a.u+=j*Math.cos(f);a.v+=j*Math.sin(f);a.xpoints.push(a.x);a.ypoints.push(a.y);a.colors.push(Math.abs(a.initSpeed-Math.sqrt(a.u*a.u+a.v*a.v))<0.1?"rgb(256, 256, 0)":Math.sqrt(a.u*a.u+a.v*a.v)>a.initSpeed*1.5?"rgb(256, 0, 0)":Math.sqrt(a.u*a.u+a.v*a.v)>a.initSpeed?"rgb(256, 128, 128)":Math.sqrt(a.u*a.u+a.v*a.v)>a.initSpeed/1.2?"rgb(64, 64, 256)":"rgb(128, 256, 256)");
a.x+=a.u;a.y+=a.v}}})();(function(){function c(d){for(var o=0,h=0,l=a;l.tagName!="BODY";){o+=a.offsetTop;h+=a.offsetLeft;l=l.offsetParent}i.setEarthMass(parseFloat(q.innerHTML));n.push(new r({x:parseInt(d.clientX-h+window.pageXOffset),y:parseInt(d.clientY-o+window.pageYOffset),u:parseFloat($(b).slider("value")),v:parseFloat($(g).slider("value")),m:parseFloat($(f).slider("value"))}))}var a,f,j,b,g,q,s,t,u;k=function(d){k.canvas=a=document.getElementById(d.canvas);f=document.getElementById(d.satmass);
j=document.getElementById(d.earthmass);b=document.getElementById(d.initx);g=document.getElementById(d.inity);q=document.getElementById(d.earthmasslbl);q.innerHTML=e.m;s=document.getElementById(d.satmasslbl);s.innerHTML=d.defaultsatmass;t=document.getElementById(d.initxlbl);t.innerHTML=d.defaultinitx;u=document.getElementById(d.initylbl);u.innerHTML=d.defaultinity;$(j).slider({slide:function(o,h){var l=parseFloat(h.value);q.innerHTML=l;i.setEarthMass(l)},value:e.m,min:1,max:100,step:1});$(f).slider({slide:function(o,
